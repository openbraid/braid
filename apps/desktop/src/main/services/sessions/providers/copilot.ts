...
// Sessions at ~/.copilot/session-state/<uuid>/
// CWD in workspace.yaml → cwd field
...
export const copilotProvider: AgentSessionProvider = {
  agent: 'Copilot CLI',
  ...
  async discoverSessions(worktreePaths: string[]): Promise<DiscoveredSession[]> {
    ...
    for (const dir of sessionDirs) {
      ...
      try {
        ...
        const matches = worktreePaths.some((wt) => parsed.cwd!.startsWith(wt))
        if (!matches) continue
        ...
        sessions.push({
          sessionId,
          agent: 'Copilot CLI',
          title: null, // Copilot doesn't store messages locally
